...
```{R}
#Randomization Test Anova version of Income and ObamaWin
set.seed(123)
obs_F<-13.88
Fs<-replicate(500,{
...
set.seed(1234)
k=10
data1<-master[sample(nrow(master)),] #put dataset in random order
folds<-cut(seq(1:nrow(master)),breaks=k,labels=F) #create folds
...
for(i in 1:k){ # FOR EACH OF 10 FOLDS
  ...
  fit<- glm(ObamaWin~BA+Income+Church+Teen+HS+CivilWar, data=train, family="binomial")## YOUR MODEL FROM ABOVE HERE, FIT TO THE TRAINING SET)
  probs<- predict(fit,newdata=test, type="response")## GET PREDICTED PROBABILITIES FROM THE MODEL USING TEST SET AS NEWDATA)
...
set.seed(1234)
k=10
data1<-masterdum[sample(nrow(masterdum)),] #put dataset in random order
folds<-cut(seq(1:nrow(masterdum)),breaks=k,labels=F) #create folds
...
for(i in 1:k){ # FOR EACH OF 10 FOLDS
  ...
  fit<- glm(ObamaWin~BA+Income+Church+Union, data=train, family="binomial")## YOUR MODEL FROM ABOVE HERE, FIT TO THE TRAINING SET)
  probs<- predict(fit,newdata=test, type="response")## GET PREDICTED PROBABILITIES FROM THE MODEL USING TEST SET AS NEWDATA)
